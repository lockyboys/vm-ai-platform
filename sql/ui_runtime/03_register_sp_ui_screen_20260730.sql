/* Register SP_UI_SCREEN through the existing Repository Object Procedure. */
USE te_story_platform;

SET @sp_ui_screen_object_id = COALESCE(
    (SELECT object_id FROM sp_object WHERE object_code = 'SP_UI_SCREEN' AND deleted_dt IS NULL LIMIT 1),
    @sp_ui_screen_object_id
);

CALL sp_register_repository_object(
    JSON_OBJECT(
        'object_id', @sp_ui_screen_object_id,
