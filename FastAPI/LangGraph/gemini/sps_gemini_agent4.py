...
8. 등록된 Verified SQL 목록을 묻는 경우에는 `verified_sql` 도구를 한 번만 호출하고, include_sql_text=true로 목록을 확인한 뒤 즉시 답변한다.
9. 같은 도구를 같은 인자로 반복 호출하지 않는다. 조회 결과가 비어 있으면 비어 있다고 답하고 다른 도구를 임의로 연쇄 호출하지 않는다.
10. 도구 호출 결과를 받은 뒤에는 추가 조회 없이 사용자가 이해할 수 있는 한국어 요약으로 답변을 완성한다.
11. `cm_verified_sql_query`와 `verified_sql`은 MariaDB Verified SQL 영역이며, MongoDB의 Collection/Document 조회와 혼동하지 않는다.
12. 질문이 MongoDB 저장 데이터나 Collection을 포함하면 MongoDB 관련 도구를 사용했는지 명시하고, 확인하지 않았으면 'MongoDB는 별도 확인이 필요하다'고 안내한다.
13. MariaDB와 MongoDB를 함께 설명할 때는 각각 확인된 사실과 미확인 사실을 분리해 안내하고, 한쪽의 조회 결과를 다른 쪽의 증거로 사용하지 않는다.
14. `mongodb_collection_stats`는 MongoDB Collection의 통계/문서 수 조회 도구이며 테이블 스키마를 조회하지 않는다. MongoDB Collection 스키마를 직접 보장하지 않는 경우 결과를 '통계'로만 표현한다.
15. MariaDB 테이블 스키마가 필요하면 `table_schema`를 사용하고, MongoDB Collection 통계가 필요하면 `mongodb_collection_stats`를 사용한다. 사용자가 도구를 잘못 지정하면 도구를 억지로 반복 호출하지 말고 이 차이를 설명한다.
16. 도구 호출에 필수 인자(예: MongoDB의 collection_name과 role)를 확인할 수 없으면 값을 추정하지 말고 필요한 인자를 질문하거나 미확인으로 안내한다.
"""
...
def build_graph(model: ChatGoogleGenerativeAI, tools: list[Any]):
    ...
    async def verifier(state: SimpleState) -> dict[str, list[Any]]:
        verification_prompt = (
            ...
            "MariaDB Verified SQL과 MongoDB Collection/Document는 별도 저장소이므로, 확인한 저장소와 확인하지 않은 저장소를 구분해 안내해라. "
...
async def invoke_agent(query: str, thread_id: str) -> tuple[str, int]:
    ...
    try:
        ...
        return (
            "MCP 연결은 정상이나 요청한 도구와 조회 대상이 맞지 않아 반복 호출을 중단했습니다. "
            "mongodb_collection_stats는 MongoDB Collection의 문서 수/통계 조회용이며 테이블 스키마 조회용이 아닙니다. "
            "MariaDB 테이블 스키마는 table_schema를 사용하고, MongoDB Collection 통계는 collection_name과 role이 필요합니다. "
            "langgraph_agent_memory의 저장소가 MariaDB인지 MongoDB인지와 MongoDB라면 role 값을 알려주시면 "
            "확인된 결과만 표로 안내하겠습니다.",
            tool_count,
        )
    ...
    return extract_message_content(content), tool_count
...
async def chat(request: ChatRequest) -> ChatResponse:
    ...
    try:
        response, tool_count = await invoke_agent(request.query, request.thread_id)
...
async def run_agent(user_query: str, thread_id: str = "sps-gemini-cli") -> None:
...
async def main() -> None:
    ...
    try:
        ...
        if args.check_mcp:
            ...
            await run_agent(get_user_query(args.query), thread_id=args.thread_id)
    except Exception as error:
        if isinstance(error, asyncio.TimeoutError):
            print(
                f"MCP_AGENT_FAILED: agent timeout ({AGENT_TIMEOUT_SECONDS}s)",
                file=sys.stderr,
            )
