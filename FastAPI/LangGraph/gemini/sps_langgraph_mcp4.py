...
async def choose_tool_with_google(
    ...
    """Gemini가 답하거나 사용할 MCP 도구 하나를 선택하게 한다."""
    ...
    response = await asyncio.to_thread(
        ...
        config=genai.types.GenerateContentConfig(
            temperature=0.2,
            tools=build_google_mcp_tools(available_tools, approved),
        ),
    )
...
async def summarize_tool_result_with_google(
    ...
    """MCP 실행 결과를 Gemini가 사용자 질문에 맞게 설명하게 한다."""
    ...
    return getattr(response, "text", "") or tool_result
