...
_PROGRAM_ID = "harness.scripts.migrate_one_table_detail"
...
def _hydrate_verified_sql_text_from_mongodb(
    common_database: CommonDatabase,
    row: dict[str, Any],
) -> dict[str, Any]:
    """Read sql_text from the migrated DOCUMENT when MariaDB payload is empty."""
    ...
    if not documents:
        raise RuntimeError(f"Verified SQL text is empty and has no MongoDB payload. query_id={row['query_id']}")
    payload = documents[0].get("payload", {}).get("verified_sql_payload")
    sql_text = payload.get("sql_text") if isinstance(payload, Mapping) else None
    if not str(sql_text or "").strip():
        raise RuntimeError(f"MongoDB Verified SQL payload is empty. query_id={row['query_id']}")
    hydrated = dict(row)
    hydrated["sql_text"] = sql_text
    return hydrated
...
def _load_verified_queries(
    ...
    for operation_name, expected_crud_type in _VERIFIED_QUERY_CRUD.items():
        ...
        row = common_database.fetch_one(
            """
            SELECT query_id, query_name, crud_type
        ...
        row = _hydrate_verified_sql_text_from_mongodb(common_database, row)
...
def migrate_one(
    *,
    ...
    try:
        ...
        # Validate the work before consuming Identifier numbers.
        ...
        # Reserve all Identifier numbers in a short, dedicated transaction.
        identifier_jobs = (
            (execution_request, execution_prepared),
            (document_request, document_prepared),
            (change_history_request, change_history_prepared),
        )
        reservations = []
        try:
            for request, prepared in identifier_jobs:
                coordinator.acquire(prepared)
                acquired_preparations.append(prepared)
                reservations.append(
                    coordinator.reserve(request=request, prepared=prepared)
                )
            ...
            acquired_preparations.clear()

        execution_history_id, document_detail_id, change_history_id = (
            coordinator.render_resolution(
                request=request,
                prepared=prepared,
                resolution=reservation,
            )
            for (request, prepared), reservation in zip(
                identifier_jobs,
                reservations,
                strict=True,
            )
        ...
        # Start the business transaction only after Identifier locks are released.
